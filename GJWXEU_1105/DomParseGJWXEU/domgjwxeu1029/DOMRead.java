...
public class DOMRead {
    ...
    {
        File xmlFile = new File("hallgatoGJWXEU.xml");

        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();

        DocumentBuilder dBuilder = factory.newDocumentBuilder();

        Document gjwxeu = dBuilder.parse(xmlFile);

        gjwxeu.getDocumentElement().normalize();

        System.out.println("Gyökér elem: " + gjwxeu.getDocumentElement().getNodeName());

        NodeList nList = gjwxeu.getElementsByTagName("hallgato");
        ...
        for (int i = 0; i < nList.getLength(); i++)
        {
            ...
            if (nNode.getNodeType() == Node.ELEMENT_NODE)
            {
                ...
                System.out.println("Kersztnév: " + kname);
