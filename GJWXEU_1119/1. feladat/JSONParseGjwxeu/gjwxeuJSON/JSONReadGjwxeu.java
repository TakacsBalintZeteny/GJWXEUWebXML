...
public class JSONReadGjwxeu {
    public staic void main(String[] args) {
        try(FileReader reader = new FileReader("orarendGjwxeu.json")) {
            //Parse
            JSONParser jsonParser = new JSONParser();
            JSONObject jsonObject = (JSONObject)jsonParser.parse(reader);

            //Root, majd óra lista lekérése
            JSONObject root = (JSONObject) jsonObject.get("GJWXEU_orarend");
            JSONArray lessons = (JSONArray) root.get("ora");

            //Óra adatok kiírása
            System.out.println("GJWXEU Órarend 2025 ősz: \n");
